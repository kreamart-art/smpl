...
import DropPads from '../components/DropPads.jsx'
...
export default function Play() {
  ...
  return (
    <div className="mx-auto max-w-[1000px] px-4 py-14 sm:px-6 sm:py-20">
      ...
      {/* the three games — predict, guess the sample, drop */}
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div data-tour="play-drop" className="group relative flex flex-col overflow-hidden border border-line bg-panel p-6">
          <span className="hover-bloom" aria-hidden="true" />
          <div className="relative">
            <DropPads size={64} />
          </div>
          <div className="relative mt-4 flex items-center gap-2">
            <span className="block h-1.5 w-1.5 bg-accent pulse-dot" />
            <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted">{t('play.dropTag')}</span>
          </div>
          <h2 className="relative mt-3 font-sans text-xl font-bold uppercase tracking-tight">{t('play.dropTitle')}</h2>
          <p className="relative mt-2 flex-1 font-mono text-[12px] leading-relaxed text-muted">{t('play.dropSub')}</p>
          <div className="relative mt-5">
            <Btn href="/drop/index.html" variant="accent">
              {t('play.dropCta')}
            </Btn>
          </div>
        </div>
