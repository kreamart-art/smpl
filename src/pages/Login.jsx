...
export default function Login() {
  ...
  const go = async (who, pw) => {
    setBusy(true)
    const r = await login(who, pw)
    setBusy(false)
          ...
          <>
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault()
                setError('')
                go(identifier.trim(), password)
              }}
              className="mt-6 space-y-4"
            >
              <Field label={t('auth.field.identifier')} hint={t('auth.field.identifierHint')}>
                <input
                  className={inputCls}
              ...
              <Btn type="submit" variant="solid" size="lg" full disabled={busy || !identifier.trim() || !password}>
